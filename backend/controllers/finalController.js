...
/////////////////////////////
const { createFinalProduct, getAllFinalProducts, updateFinalProduct, deleteFinalProduct } = require('../models/finalModel');

// Add final product
const addFinalProduct = async (req, res) => {
  const { pname, batch_no, quantity, expiry_date, storage_zone_id } = req.body;

  if (!pname || !batch_no || !quantity || !expiry_date || !storage_zone_id) {
    return res.status(400).json({ error: 'All fields are required.' });
  }

  try {
    const newFinalProduct = await createFinalProduct(pname, batch_no, quantity, expiry_date, storage_zone_id);
    res.status(201).json({ message: 'Final product created successfully', finalProduct: newFinalProduct });
  } catch (error) {
...
const updateFinalDetails = async (req, res) => {
  const { fproduct_id, pname, batch_no, quantity, expiry_date, storage_zone_id } = req.body;

  if (!fproduct_id || !pname || !batch_no || !quantity || !expiry_date || !storage_zone_id) {
    return res.status(400).json({ error: 'All fields are required.' });
  }

  try {
    const updatedFinal = await updateFinalProduct(fproduct_id, pname, batch_no, quantity, expiry_date, storage_zone_id);
    res.status(200).json({ message: 'Final product updated successfully', finalProduct: updatedFinal });
  } catch (error) {
    console.error(error.message);
    res.status(400).json({ error: error.message });
