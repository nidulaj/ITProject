                ...
                onClick={async () => {
                  try {
                    ...
                    const response = await axios.put(`${API}/api/ingredient-totals/reduce`, {
                      icode_id: 2, // Test with milk (ICD002)
                      quantity: 1
                    });
                    setSuccess("Test reduce successful! " + JSON.stringify(response.data));
                    await loadTotals();
                  } catch (e) {
                    setError("Test reduce failed: " + (e?.response?.data?.error || e.message));
                  } finally {
                    setLoading(false);
                  }
                }}
